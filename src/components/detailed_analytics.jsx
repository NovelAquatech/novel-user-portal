...
import { getOrganizedSensorData, organizedExportedData, capitalizeFirstLetter } from "../helper/utils";
...
export const DetailedAnalytics = React.forwardRef(
  (
            ...
            convertToBase64(orgIcon)
              ...
              .finally(() => {
                const hiddenDetails = [
                  ...
                  { text: `Report Type: ${selectedParam.map(item => item.label).join(', ')}`, x: 10, y: 40 },
                ];
        ...
        try {
          const columns = [
            ...
            ...displayParameters.map((parameter) => {
              return { title: capitalizeFirstLetter(parameter), dataKey: parameter }
            })
          ];
          const filterData = organizedExportedData(sensorData, selectedHourly, parameters, selectedDevices);
          if (filterData) {
    ...
    const updateParamName = (paramDisplayName) => {
      if(paramDisplayName.toLowerCase() == 'tvoc') return paramDisplayName.toUpperCase();
    ...
    const multiSelectOptions = displayParameters.map((parameter) => {
      return {
        label: capitalizeFirstLetter(updateParamName(parameter)), value: parameter
      }
    })
